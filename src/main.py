...
def generate_agents():
    global agents
    for i in range(50):
        agents.add(traits.Organism(np.random.randint(20,50),np.random.randint(400),np.random.randint(50),np.random.randint(200),pos=(np.random.randint(800),np.random.randint(800)),health=np.random.randint(2000,5000),env=env))
...
def assign_species_names():
    ...
    X = [agent.color for agent in agents]
    """for agent in agents:
        print(agent)
    print(X)"""
...
def create_background():
    ...
    for row in range(rows):
        ...
        for column in range(int(cols/2)):
            ...
            block = traits.Terrain("land", column * (block_width), row * block_height)
            terrain.add(block)
        for column in range(int(cols/2),cols):
            # Create a block (color,x,y)
            block = traits.Terrain("water", column * (block_width), row * block_height)
            terrain.add(block)
...
while True:
    ...
    counter += 1
    if n_born % 100 == 0 :# change to 0 when ready to implement
